...
const Main = require('../models/main'); // Use Main model instead of Task
...
router.get('/tasks-with-reminders', async (req, res) => {
  try {
    ...
    const tasksWithReminders = await Main.findAll({
      ...
      order: [
        ['createdAt', 'DESC'], // Order tasks by creation date (descending) to show the most recent task on top
      ],
    });
...
router.get('/completed-tasks', async (req, res) => {
  try {
    ...
    const completedTasks = await Main.findAll({
      ...
      order: [
        ['statusUpdatedAt', 'DESC'], // Order tasks by statusUpdatedAt (descending) to show the most recently completed task on top
      ],
    });
...
router.get('/cancelled-tasks', async (req, res) => {
  try {
    ...
    const cancelledTasks = await Main.findAll({
      ...
      order: [
        ['statusUpdatedAt', 'DESC'], // Order by statusUpdatedAt in descending order to get the most recent cancellations first
      ],
    });
...
router.get('/incompleted-tasks', async (req, res) => {
  try {
    ...
    const incompletedTasks = await Main.findAll({
      ...
      order: [
        ['statusUpdatedAt', 'DESC'], // Order by statusUpdatedAt in descending order to get the most recent incompleted tasks first
      ],
    });
